...
import { LPNav, Footer, PG } from './site-chrome';
...
/**
 * `initialCurrency` comes from the visitor's geo header, resolved on the server
 * in app/page.tsx. It matters that this is server-side: the prices are in the
 * initial HTML, so deciding currency after mount makes an India visitor watch
 * dollar prices repaint into rupees. Null means the header was absent (local
 * dev, or a host that doesn't geolocate) — only then do we guess from timezone.
 */
export default function LandingPage({ initialCurrency }: { initialCurrency: Currency | null }) {
  const [currency, setCurrency] = React.useState<Currency>(initialCurrency ?? 'USD');
  React.useEffect(() => {
    if (!initialCurrency) setCurrency(detectCurrency());
  }, [initialCurrency]);
  return (
    <>
