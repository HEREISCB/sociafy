...
const securityHeaders = [
  ...
  { key: 'Strict-Transport-Security', value: 'max-age=63072000; includeSubDomains; preload' },
  {
    ...
    value: [
      "default-src 'self'",
      `script-src 'self' 'unsafe-inline' 'unsafe-eval' ${clerkScript} https://challenges.cloudflare.com https://static.cloudflareinsights.com`,
      "worker-src 'self' blob:",
      "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com",
      "font-src 'self' https://fonts.gstatic.com data:",
      "img-src 'self' data: blob: https:",
      "media-src 'self' blob: https:",
      `connect-src 'self' ${clerkScript} https://api.anthropic.com https://cloudflareinsights.com`,
      `frame-src 'self' https://challenges.cloudflare.com https://*.clerk.com${clerkOrigin ? ` ${clerkOrigin}` : ''}`,
